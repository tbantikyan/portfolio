<template>
  <!-- outer div controls whether in dark mode-->
    <div
      class="drop-shadow max-w-screen fixed left-0 top-0 flex h-20 w-screen items-center justify-between bg-inherit px-8 text-3xl text-inherit dark:bg-inherit dark:text-inherit sm:px-10 md:px-12 lg:h-16 lg:px-20"
    >
      <!-- site logo -->
      <div
      ...
      >
        ...
        >
          ...
          >
            <li
              class="pb-4 sm:flex sm:h-full sm:place-items-center sm:pb-0 sm:border-t-transparent"
              :class="{
                'text-indigo-500 sm:border-y-4 sm sm:border-b-indigo-500 sm:border-t-transparent transition ease-linear':
                  isActive('/'),
              }"
            >
